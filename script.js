...
const okButtonElement = document.querySelector('.ok-button');
const mainObjectElement = document.querySelector('.main-object');
let highScore = 100000; // 매우 큰 값으로 초기화
localStorage.setItem('highScore', highScore);
const startGame = () => {
    buttonElement.disabled = true; // 버튼 비활성화
    typedValueElement.disabled = false; // 입력 필드 활성화
    mainObjectElement.style.display = 'block'; // 메인 오브젝트 숨기기
    modalElement.style.display = 'none'; // 모달 숨기기
    if (localStorage.getItem('highScore')) {
    highScore = parseFloat(localStorage.getItem('highScore')) || highScore;
}
};
const gameEnd = () => {
    buttonElement.disabled = false; // 버튼 활성화
    typedValueElement.disabled = true; // 입력 필드 비활성화
    modalElement.style.display = 'block'; // 모달 표시
    mainObjectElement.style.display = 'none'; // 메인 오브젝트 숨기기

    const elapsedTime = new Date().getTime() - startTime;
    scoreElement.innerText = `Your Score: ${elapsedTime / 1000} seconds`;
    storageHighScore = highScore < elapsedTime / 1000 ? highScore : (elapsedTime / 1000);
    localStorage.setItem('highScore', storageHighScore);
    console.log(`Current High Score: ${storageHighScore} seconds`);
...
const initGame = () => {
    gameEnd(); // 초기 상태 설정
    modalElement.style.display = 'none'; // 모달 숨기기
    mainObjectElement.style.display = 'block';
};
initGame();
...
typedValueElement.addEventListener('input', () => {
    ...
    if (typedValue === currentWord && wordIndex === words.length - 1) { // 마지막 단어까지 정확히 입력했는 지 체크
        ...
        const message = `최고 기록은 ${localStorage.getItem('highScore')}초입니다.`; // 타이핑 완료 메시지
    ...
    typedValueElement.style.background = randomColor();
})
